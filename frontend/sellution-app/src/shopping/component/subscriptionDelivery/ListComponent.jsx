...
const ListComponent = () => {
  const { isListBlurred, subscriptionDeliveryProductList } = useList();
  const { clientName } = useClientName();
  //정기배송 상품 더미데이터
  return (
    <>
      <MenuCategoryHeaderNav title={'정기배송 상품 목록'} />
      <main className={`main-box w-full ${isListBlurred ? 'blurred' : ''}`}>
        <ul className='product-list w-[90%] mx-auto bg-white list-none p-0'>
          {subscriptionDeliveryProductList.map((product) => (
