    ...
    async (error) => {
      ...
      const refreshAccessToken = async () => {
        try {
          ...
          const shopCompanyStorage = JSON.parse(localStorage.getItem('shop-company-storage'));
          const shopUrl =
            shopCompanyStorage?.state?.name !== '' ? shopCompanyStorage?.state?.name : 'error';
          ...
          if (error.response?.data?.code === 4005) {
            ...
            setAccessToken(null);
            window.location.href = `/shopping/${shopUrl}/login`;
