...
public class CustomerProductController {
    ...
    @GetMapping
    public ResponseEntity<List<FindCustomerProductRes>> findAllProducts(@PathVariable String shopUrl, //url 경로에서 companyId 추출
                                                                        @RequestParam(value="deliveryType", required = true) DeliveryType deliveryType,
                                                                        @RequestParam(value="categoryId", required = false) Long categoryId,
                                                                        Pageable pageable){
        //Long companyId = companyService.getCompanyIdByShopUrl(shopUrl);
        //리턴
        return ResponseEntity.ok(customerProductService.findAllProducts(shopUrl, deliveryType, categoryId));
    }
