...
//    @Scheduled(cron = "0 0/1 * * * *")
    ...
    public int regularRetryPayment() {
        ...
        for (Order order : ordersForRetry) {
            ...
            PaymentHistory paymentHistory = paymentHistoryRepository.findPendingPaymentHistory(orderId, yesterday.atStartOfDay());
    ...
    public int regularDelivery() {
        ...
        for (Order order : ordersForDelivery) {
            ...
            if (paymentHistory != null && paymentHistory.getStatus() == PaymentStatus.COMPLETE) {
                ...
                {
                    // 7.1 월정기주문이면 다음 배송일을 (오늘이후) ~ (마지막 배송일) 사이에서 찾는다.
                    case MONTH_SUBSCRIPTION:
                        order.updateNextDeliveryDate(paymentUtil.calculateNextDeliveryDateForMonthOrder(LocalDate.now(),order.getDeliveryEndDate(),order));
                        break;

                    case COUNT_SUBSCRIPTION:
                        order.updateNextDeliveryDate(paymentUtil.calculateNextDeliveryDateForCountOrder(LocalDate.now(),order));
                        break;
                }
            }
