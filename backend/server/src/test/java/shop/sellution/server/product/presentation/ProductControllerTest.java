...
class ProductControllerTest extends BaseControllerTest {
    ...
    void getAllProducts_Success() throws Exception {
        ...
        when(productService.getAllProducts(anyLong(), any(Pageable.class), anyString(), anyString(), anyString(), anyString(), anyString()))
                .thenReturn(new PageImpl<>(List.of(findProductRes), pageable, 1));

        mockMvc.perform(get("/products")
                        .param("companyId", "1")
                        .param("page", "0")
                        .param("size", "10")
                        .param("deliveryType", "전체")
                        .param("isDiscount", "전체")
                        .param("categoryName", "전체")
                        .param("isVisible", "전체")
                        .param("productName", ""))
                ...
                .andDo(document("Product/findAllProducts",
                        ...
                        queryParameters(
                                ...
                                parameterWithName("deliveryType").description("배송 타입").optional(),
                                parameterWithName("isDiscount").description("할인 여부").optional(),
                                parameterWithName("categoryName").description("카테고리 이름").optional(),
                                parameterWithName("isVisible").description("노출 여부").optional(),
                        ...
                        responseFields(
                                ...
                                fieldWithPath("content[].cost").type(JsonFieldType.NUMBER).description("가격"),
                                ...
                                fieldWithPath("content[].previousMonthSales").type(JsonFieldType.NUMBER).description("이전 달 판매량"),
                                fieldWithPath("content[].isDiscount").type(JsonFieldType.STRING).description("할인 여부").optional(),
                                fieldWithPath("content[].discountRate").type(JsonFieldType.NUMBER).description("할인율"),
                                fieldWithPath("content[].discountedPrice").type(JsonFieldType.NUMBER).description("할인된 가격"),
        ...
        verify(productService, times(1)).getAllProducts(anyLong(), any(Pageable.class), anyString(), anyString(), anyString(), anyString(), anyString());
    }
    ...
    void deleteProducts_Success() throws Exception {
        ...
        doNothing().when(productService).deleteProducts(anyList());
        ...
        mockMvc.perform(delete("/products")
                ...
                .andExpect(status().isNoContent())
                .andDo(document("Product/deleteProducts",
                        preprocessRequest(prettyPrint()),
                        preprocessResponse(prettyPrint()),
                        requestFields(
                                fieldWithPath("[]").type(JsonFieldType.ARRAY).description("삭제할 상품 ID 목록")
                        )
                ));

        verify(productService, times(1)).deleteProducts(anyList());
    }
}
